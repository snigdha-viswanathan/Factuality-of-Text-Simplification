...
def score_example_single_context(decode_text, input_text, model, tokenizer, args):
    ...
    if input_ids.shape == torch.Size([0]) or attention.shape==torch.Size([0]):
        error_source.append(input_text)
        error_simplified.append(decode_text)
        return 0
    ...
    # if len(inputs['input_ids'])==0 or len(inputs['attention'])==0:
...
def main():
    ...
    if args.test_type == 'summ':
        test_file = json.load(
            open('./resources/pegasus_baseline_test_pred.json'))
        ...
        for ex in test_file:
            article_sent = str(ex['article_sent'])
            correct_sent = str(ex['correct_sent'])
            # incorrect_sent = ex['incorrect_sent']

            if article_sent.isspace() or len(article_sent)==0 or len(correct_sent)==0 or correct_sent.isspace():
                # print("correct")
                score1=0
                source_sentence.append(article_sent)
                simplified_sentence.append(correct_sent)
    ...
    df['source_sentence']=source_sentence
    ...
    # error_df['simplified_sentence']=error_simplified
    
    # error_df.to_csv('./resources/output_error.csv')
